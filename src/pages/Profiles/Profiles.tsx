...
export default function Profiles() {
  ...
  const filtered = (accounts || []).filter(acc => {
    ...
    const deviceId = (acc?.associated_device_id || "").toLowerCase();
    const searchTerm = (search || "").toLowerCase();
    return username.includes(searchTerm) || deviceId.includes(searchTerm);
  });

  // EXCLUDE ADMINS: Only show Primary Homeowners as top level
  const topLevelAccounts = filtered.filter(acc => acc?.role === "homeowner_primary");

  // EXCLUDE ADMINS from summary counts
  const homeownersOnly = (accounts || []).filter(a => a?.role !== "admin");
  const totalPrimary = homeownersOnly.filter(a => a?.role === "homeowner_primary").length;
  const totalSecondary = homeownersOnly.filter(a => a?.role === "homeowner_invited").length;
        ...
        <div className="flex flex-col md:flex-row justify-between items-start md:items-end gap-8">
          ...
          <div className="accounts-summary">
            <div className="summary-item">
              <span className="summary-label">Primary Owners</span>
              <span className="summary-value accent">{loading ? ".." : totalPrimary}</span>
            </div>
            <div className="summary-item border-l border-white/5 pl-8">
              <span className="summary-label">Invited Guests</span>
              <span className="summary-value text-white">{loading ? ".." : totalSecondary}</span>
            </div>
            <div className="summary-item border-l border-white/5 pl-8">
                    ...
                    <td>
                      <div className="user-identity-cell">
                        ...
                        <div className="user-info-stack">
                          ...
                          <span className="user-role-tag">Primary User</span>
                        </div>
                      </div>
                    </td>
                    <td>
